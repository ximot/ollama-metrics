...
- `MAX_CONCURRENT_REQUESTS` - Optional maximum number of proxied requests handled concurrently. When set, additional requests wait in the proxy queue and are exposed via `ollama_queued_requests`.
...
- `ollama_inflight_requests` - Number of proxied requests currently being handled
- `ollama_queued_requests` - Number of proxied requests waiting for a concurrency slot when `MAX_CONCURRENT_REQUESTS` is set
